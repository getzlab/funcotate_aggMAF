...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Merging SNP on-cis from MuTect1 result to DNP/TNP/ONP"
    )
    ...
    parser.add_argument(
        "-o_vcf",
        required=True,
        help="Path to output VCF file that contains SNP/INDEL/XNP",
    )
...
if __name__ == "__main__":
    ...
    temp = (
        smaf.assign(ID=".", QUAL=".", FILTER="PASS", FORMAT="AD")
        .loc[:, vcf_header]
        .replace("-", ".")
    )
    ...
    for i, r in temp.iterrows():
        if r["REF"] == ".":  # insertion
            temp.at[i, "REF"] = get_refbase(
                REF_FA, r["#CHROM"], int(r["POS"]) - 1
            ).capitalize()
            temp.at[i, "ALT"] = (
                get_refbase(REF_FA, r["#CHROM"], int(r["POS"]) - 1).capitalize()
                + r["ALT"]
            )
        elif r["ALT"] == ".":  # deletion
            temp.at[i, "REF"] = (
                get_refbase(REF_FA, r["#CHROM"], int(r["POS"]) - 2).capitalize()
                + r["REF"]
            )
            temp.at[i, "ALT"] = get_refbase(
                REF_FA, r["#CHROM"], int(r["POS"]) - 2
            ).capitalize()
